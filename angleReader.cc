...
MStatus angleReader::initialize() {
    ...
    MGlobal::displayInfo(MString("Initialize!"));
    // Function sets
    ...
    // start
    start_uAttr = uAttr.create("start", "str", MFnUnitAttribute::kAngle, 0.2);
    CHECK_MSTATUS(uAttr.setKeyable(true));

    // end
    end_uAttr = uAttr.create("end", "end", MFnUnitAttribute::kAngle, 0.5);
    CHECK_MSTATUS(uAttr.setKeyable(true));
        
    // postEnd
    postEnd_uAttr = uAttr.create("postEnd", "sta", MFnUnitAttribute::kAngle, 0.75);
    CHECK_MSTATUS(uAttr.setKeyable(true));
...
MStatus angleReader::compute(const MPlug& plug, MDataBlock& data){
    MGlobal::displayInfo(MString("Computing!"));

    // if (plug != outAngle_nAttr && plug != angleReader.outWeight_nAttr){
	...
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'baseMatrix' attribute");
	// MMatrix base_M = baseMatrix_DH.asMatrix();

    // // driver_M
    // MDataHandle driverMatrix_DH = data.inputValue(driverMatrix_mAttr, &stat);
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'driverMatrix' attribute");
	// MMatrix driver_M = driverMatrix_DH.asMatrix();

    // // rotateAxis_V
    // MDataHandle rotateAxisX_DH = data.inputValue(rotateAxisX_nAttr, &stat);
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'rotateAxisX' attribute");
    // MDataHandle rotateAxisY_DH = data.inputValue(rotateAxisY_nAttr, &stat);
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'rotateAxisY' attribute");
    // MDataHandle rotateAxisZ_DH = data.inputValue(rotateAxisZ_nAttr, &stat);
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'rotateAxisZ' attribute");
	...
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'frontAxisX' attribute");
    // MDataHandle frontAxisY_DH = data.inputValue(frontAxisY_nAttr, &stat);
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'frontAxisY' attribute");
    // MDataHandle frontAxisZ_DH = data.inputValue(frontAxisZ_nAttr, &stat);
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'frontAxisZ' attribute");
    ...
    // // compute the angle
    // double angle_d = atan2(dotProjBaseFront, dotProjUp);
	// MAngle outAngle_A(angle_d, MAngle::kRadians);
	// MDataHandle outAngle_DH = data.outputValue(outAngle_uAttr, &stat);
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'outAngle' attribute");
	// outAngle_DH.setMAngle(outAngle_A);
	// outAngle_DH.setClean();

    // // compute the weight
    // MDataHandle outWeight_DH = dataBlock.outputValue(outWeight_nAttr, &stat);
	// CHECK_MSTATUS(stat, "failed to get MDataHandle for 'outWeight' attribute");
    // double weight_d = computeWeight(angle, preStart, start, end, postEnd, negate);
    // outWeight_DH.setDouble(weight);
...
MBoundingBox angleReader::boundingBox() const{
